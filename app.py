...
def create_session():
    ...
    result = sessions_col.insert_one({
        ...
        'priority_index':   data.get('priority_index'),      # int or None
        'priority_text':    data.get('priority_text', ''),
        'allowed_domains':  data.get('allowed_domains', []), # list of domain strings
...
@app.route('/api/daily-log/<date_str>', methods=['GET'])
def get_daily_log(date_str):
    """Full snapshot of one day: plan + sessions + review."""
    plan     = plans_col.find_one({'date': date_str})
    review   = reviews_col.find_one({'date': date_str})
    sessions = list(sessions_col.find({'date': date_str}, sort=[('start_time', 1)]))
    return jsonify({
